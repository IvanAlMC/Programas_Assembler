...
;Macro para generar un numero aleatorio de 0 a 4
%macro generate_random 0
    pusha
    mov     ecx, 0xff   ;Carga el valor hexadecimal 0xFF (255 en decimal) en ecx
    rdtsc               ;Lee el contador de tiempo de CPU 
    xor     edx, edx    ;Se limipa edx
    and     eax, ecx    ;limita el valor en eax a un rango de 0 a 255.
    imul    eax, 5      ;Se deja aca el 5, para el que cociente nos pueda dar: 0,1,2,3 o 4
    mov     edi, eax    ;Mueve el resultado de la multiplicación al registro edi
    idiv    ecx         ;Divide el valor en el registro eax
    mov     [number], eax ;Se guarda el cociente de la division en la variable number
    popa
%endmacro
...
msg_input_s     db      "El modo seleccionado fue: Ingresar jugador", 0  ;Mensaje para opcion ingresar por usuario el jugador
msg_random_s    db      "El modo seleccionado fue: Jugador aleatorio", 0 ;Mensaje para opcion: jugador aleatorio

;Mensaje para opcion para escojer primer jugador
msg_f_menu db  '  ╔══════════════════════════╗', 0Ah,  '  ║      ¿Cómo iniciar?      ║', 0Ah,  '  ╠════╦═════════════════════╣', 0Ah,  '  ║ 1  ║ Seleccionar jugador ║', 0Ah,  '  ╠════╬═════════════════════╣', 0Ah,  '  ║ 2  ║  Jugador aleatorio  ║', 0Ah,  '  ╚════╩═════════════════════╝', 0Ah, 'Ingresa tu opcion: ', 0
...
;Se muestra como escojer al primer jugador
first_menu:

        ;Mensjae para el primer menu
        mov     eax, msg_f_menu
        call    print_string
        call    print_nl
        call    print_nl
        ;Validador de opciones para seeccionar jugador
        call    read_int        
        cmp     eax, 1
        je      input_selected
        cmp     eax, 2
...
;En caso de que se quiere empezar por un jugador aleatorio
random_selected:

        ;Se genera el indice del jugador aleatorio
        generate_random
        ...
        ;Si mueve al indice el numero aleatorio
        mov     dword [ind_player], eax ;Comienza por un jugador random
        jmp     after_choose

;En caso de que se quiere empiezar por un jugador ingresado
input_selected:
...
after_choose:
;Se inicia los valores demas valores para iniciar el juego
...
loop_estribillo_one:
        ...
        mov     ebx, 1500000000   ; Establecer el valor de ebx para el bucle delay_loop
